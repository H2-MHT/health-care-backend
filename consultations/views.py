...
from utils.prescription_translation import (
    translate_prescription_content, 
    translate_consultaion_summary
)
...
def send_pdf_via_sendgrid(pdf_content, temp_pdf_name, recipient_email):
...
def send_prescription_email(request, prescription):
    try:
        ...
        context = {
            ...
            'medicines': copy.deepcopy(prescription.medicines),
        ...
        # pdf generated to email
        pdf_file_to_email, temp_pdf_name_to_email = generate_pdf(template_path, context, request)
        
        # pdf genreated to database 
        copied_context['qr_code_base64'] = qr_code_base64
        pdf_file_to_db, temp_pdf_name_to_db = generate_pdf(template_path, copied_context, request)
        prescription.pdf_file.save(temp_pdf_name_to_db, ContentFile(pdf_file_to_db))
     
        send_pdf_via_sendgrid(pdf_file_to_email, temp_pdf_name_to_email, patient_user.email)
...
def generate_consultation_pdf(template_path: str, context: dict, request) -> tuple:
    html_string = render_to_string(template_path, context)
    pdf_file = BytesIO()

    HTML(string=html_string, base_url=request.build_absolute_uri()).write_pdf(pdf_file)

    folder_path = os.path.join(settings.MEDIA_ROOT, 'consultations')
    os.makedirs(folder_path, exist_ok=True)

    filename = f"consultation_{context['appointment_id']}.pdf"
    file_path = os.path.join(folder_path, filename)

    with open(file_path, 'wb') as f:
        f.write(pdf_file.getvalue())

    media_url = f"{settings.MEDIA_URL}consultations/{filename}"
    return pdf_file.getvalue(), media_url


def generate_consulation_summary(request, consultation, user):
    context = {
                ...
                "summary": consultation.translated_text,
                "recommendation": consultation.recommendation,
                "date": consultation.created_at.strftime("%Y-%m-%d"),
                "consultation_summary_html": "Consultation Summary",
                "patient_html": "Patient Name",
                "doctor_html": "Doctor Name",
                "appointment_id_html": "Appointment ID",
                "date_html": "Date",
                "short_description_html": "Short Description",
                "summary_html": "Summary",
                "recommendation_html": "Recommendation",
                "generated_by_html": "Generated by Your Healthcare System",
    }
    try:
        language = AppLanguage.objects.get(user=user)
        lang_code = language.code
    except AppLanguage.DoesNotExist:
        lang_code = "en"

    context = translate_consultaion_summary(context, lang_code)
    pdf_bytes, pdf_url = generate_consultation_pdf("consultation.html", context, request)
    return pdf_bytes, pdf_url


def send_consultation_email(user, pdf_bytes, pdf_url, subject="Your Consultation Summary"):
    ...
    message = Mail(
        ...
        subject=subject,
        plain_text_content="Please find attached your consultation summary."
    )
    attachment = Attachment(
        FileContent(encoded_pdf),
...
def consultation_report(consultation_id, request):
        ...
        try:
             #  Generate English PDF for DB storage
            db_context = {
                "patient_name": consultation.patient.user.get_full_name(),
                "doctor_name": consultation.doctor.user.get_full_name(),
                "appointment_id": consultation.appointment.id,
            ...
            english_context = translate_consultaion_summary(db_context.copy(), 'en')
            english_pdf_bytes, _ = generate_consultation_pdf("consultation.html", english_context, request)

            # Save using ContentFile
            filename = f"consultation_{consultation.appointment.id}.pdf"
            consultation.consultation_report.save(filename, ContentFile(english_pdf_bytes))
            consultation.save()
    
            # Generate PDF and send email for doctor
            doctor_user = consultation.doctor.user
            doctor_pdf_bytes, doctor_pdf_url = generate_consulation_summary(request, consultation, doctor_user)
            send_consultation_email(doctor_user, doctor_pdf_bytes, doctor_pdf_url, subject="Consultation Summary for Doctor")
    
            # Generate PDF and send email for patient
            patient_user = consultation.patient.user
            patient_pdf_bytes, patient_pdf_url = generate_consulation_summary(request, consultation, patient_user)
            send_consultation_email(patient_user, patient_pdf_bytes, patient_pdf_url, subject="Consultation Summary for Patient")

            return Response({'message': 'Consultation summaries emailed successfully.'})
